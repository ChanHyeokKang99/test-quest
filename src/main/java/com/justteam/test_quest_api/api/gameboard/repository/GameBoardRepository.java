...
public interface GameBoardRepository extends JpaRepository<GameBoard, String> {
    ...
    @Query("SELECT new com.justteam.test_quest_api.api.gameboard.dto.GameBoardDetailSummaryDto(" +
            ...
            "g.startDate, g.endDate, g.views, g.createAt, g.recruitStatus, g.user.nickname, g.user.userId)" +
            "FROM GameBoard g " +
            "WHERE g.id=:boardId")
    ...
    @Query("SELECT new com.justteam.test_quest_api.api.gameboard.dto.GameBoardSummaryDto(g.id, g.title, g.platform, g.type, g.thumbnailUrl, g.startDate, g.endDate, g.views, g.createAt, g.user.nickname)" +
            "FROM GameBoard g " +
